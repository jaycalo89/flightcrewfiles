...
community = count('community_feed.json', 'posts')
...
with open('.verify_result.json', 'w', encoding='utf-8') as f:
    json.dump({'videos': videos, 'news': news, 'uap': uap, 'community': community, 'sitemap_ok': sitemap_ok}, f)

print(f"Verify: videos={videos} news={news} uap={uap} community={community} sitemap_ok={sitemap_ok}")
PYEOF
  ...
  COMMUNITY_COUNT=$(python3 -c "import json;print(json.load(open('$VERIFY_FILE'))['community'])")
  ...
  if [ "$COMMUNITY_COUNT" -eq 0 ] 2>/dev/null; then
    log "WARN  community_feed.json is empty this run (non-fatal — Hacker News or Stack Exchange may be unreachable)"
  fi
...
main() {
  ...
  CRITICAL_FAILURE=0
  VIDEOS_COUNT=0; NEWS_COUNT=0; UAP_COUNT=0; COMMUNITY_COUNT=0; SITEMAP_OK=False
  verify_results
  ...
  local community_note=""
  [ "$COMMUNITY_COUNT" -eq 0 ] 2>/dev/null && community_note=" ⚠️ (empty this run)"
...
Community posts: ${COMMUNITY_COUNT}${community_note}
